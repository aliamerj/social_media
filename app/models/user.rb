...
class User < ApplicationRecord
  ...
  has_many :followers, through: :friendships, source: :user, dependent: :destroy
  has_many :followings, through: :friendships, source: :friend, dependent: :destroy
  ...
  def like!(post)
    ...
    # this similar to self.likes
    likes << Like.new(post: post)
  end

  # Define a helper method to follow
  def follow!(user)
    # this similar to self.friendships
    unless friendships.exists?(friend_id: user.id)
